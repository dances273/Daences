...
namespace Kasse
{
    public partial class Alap : Form
    {
        ...
        public class AdatbázisQleDb
        {
            ...
            public void Tagfelvétel(string kódszám, string Név, string jelszo)//Func<object, EventArgs, string> Radbutton_CheckedChanged)
            {
                try
                {
                    conn.Open();
                    ...
                    /*string query = String.Format("INSERT INTO  (Azonosito,Nev,Jelszo,Beosztas) VALUES ('{0}', '{1}', '{2}', '{3}')", kódszám, Név, jelszo);// Radbutton_CheckedChanged);*/
                    oda = new OleDbDataAdapter(query, conn);
                    ds = new DataSet();
                    oda.Fill(ds);
                }
