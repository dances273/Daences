...
namespace Credit_card
{
    public partial class Form1 : Form
    {
        ...
        public class AdatbázisQleDb
        {
            ...
            public void Mentes(string Kartya_tipus, int Kartya_szama,bool valodi,string ervenyesseg,string tulajdonos)
            {
                try
                {
                    conn.Open();
                    string query = String.Format("INSERT INTO Credit_card (Kartya_tipus, Kartya_szama, Valodi, Ervenyessegi_ido, Tulajdonos) VALUES ('{0}',{1},'{2}','{3}','{4}');", Kartya_tipus, Convert.ToInt32(Kartya_szama), Convert.ToBoolean(valodi), ervenyesseg, tulajdonos);
        ...
        private void Mentes_button_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                odb.Mentes(tipus, Convert.ToInt32(cardnumber_textbox.Text), Valódi_checkbox.Checked, Convert.ToString(dateTimePicker1.Value), Namelabel.Text);
            }
            catch (Exception)
            {
